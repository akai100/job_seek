...
步骤 2：如果禁用多优先级任务同时运行（configRUN_MULTIPLE_PRIORITIES）：

步骤 2.1：如果任务优先级大于等于就绪任务最高优先级：跳转到步骤 4；

步骤 3：如果启用多优先级任务同时运行（configRUN_MULTIPLE_PRIORITIES）：

步骤 3.1：如果任务非运行中：跳转到步骤 4；

步骤 4：最低强占优先级等于任务优先级-1；

步骤 5：遍历所有核：

步骤 5.1：获取当前核上正在运行的任务的优先级；

步骤 5.2：如果当前任务未IDLE任务：当前正在运行任务的优先级需要 -1；

步骤 5.2.1：如果当前当前核上正在运行的任务处于运行态并且当前核不需要延迟切换任务上下文：

步骤 5.2.1.1：如果禁用多优先级任务同时运行并且任务不在运行中，跳转到步骤

步骤 5.2.1.2：如果开启多优先级任务同时运行，继续向下执行

步骤 5.2.1.3：如果当前核上任务优先级小于等于最低强占优先级：

步骤 5.2.1.3.1：如果开启核亲和度并且任务亲和度包括当前核：
...
步骤 1.2：将被挂起任务从状态链（就绪/延迟）移除；

步骤 1.3：如果挂起任务在事件链上：从事件链移除任务；

步骤 1.4：添加挂起任务到挂起任务链；

步骤 1.5 如果支持任务通知：

步骤 1.5.1：如果挂起任务通知状态为等待通知态，重新设置状态为未等待通知态；

步骤 1.6：如果配置多核：

步骤 1.6.1：如果调度器运行中：

步骤 1.6.1.1：如果被挂起任务处于运行中：

步骤 1.6.1.1.1：如果被挂起任务运行核为当前核，调用```vTaskYieldWithinAPI```让出CPU控制权；

步骤 1.6.1.1.2：否则：调用```prvYieldCore```接口，让指定核切换任务上下文；
...
步骤 3.1.1.1：调用```prvResetNextTaskUnblockTime```接口更新下一个任务唤醒时间；

步骤 3.1.1.2：退出临界区；

步骤 3.2：如果挂起任务等于当前运行任务：

步骤 3.2.1：如果调度器运行中：调用```portYIELD_WITHIN_API```让出CPU控制权；

步骤 3.2.2：如果调度器挂起中：

步骤 3.2.2.1：如果挂起任务数量等于当前任务数量：当前运行任务指向空；

步骤 3.2.2.2：否则：调用```vTaskSwitchContext```切换上下文；
...
步骤 3.1.3：调用```taskYIELD_ANY_CORE_IF_USING_PREEMPTION```发起任务强占；
